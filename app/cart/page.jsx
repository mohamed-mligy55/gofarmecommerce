...
const Page = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-10 bg-white min-h-screen font-sans">
            {/* Header */}
            <div className="flex items-baseline justify-between mb-12 border-b border-gray-100 pb-6">
                ...
                <p className="text-gray-400 font-medium">{products.length} items</p>
            </div>

            {products.length > 0 ? (
                <div className="flex flex-col lg:flex-row gap-16 items-start">
                    
                    {/* 1. قائمة المنتجات (Clean Design) */}
                    <div className="flex-[2] w-full">
                        <div className="divide-y divide-gray-100">
                            {products.map((product) => (
                                <div key={product.id} className="flex flex-col sm:flex-row items-center py-8 group transition-all">
                                    {/* Image */}
                                    <div className="w-32 h-32 bg-[#f9f9f9] rounded-2xl p-4 flex items-center justify-center transition-transform group-hover:scale-105">
                                        <img src={product.thumbnail || product.image} alt={product.title} className="max-h-full object-contain mix-blend-multiply" />
                                    </div>
                                    ...
                                    <div className="flex-1 sm:ml-8 mt-4 sm:mt-0 w-full">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <h3 className="text-lg font-bold text-gray-900 group-hover:text-green-600 transition-colors cursor-default">{product.title}</h3>
                                                <p className="text-sm text-gray-400 mt-1 uppercase tracking-widest text-[10px] font-bold">{product.category}</p>
                                            </div>
                                            <button onClick={() => handleRemove(product)} className="text-gray-300 hover:text-red-500 transition-all p-2">
                                        ...
                                        <div className="flex items-center justify-between mt-6">
                                            ...
                                            <div className="flex items-center border border-gray-100 rounded-full bg-white shadow-sm px-2 py-1">
                                                <button onClick={() => dispatch(decreaseQuantity(product))} className="w-8 h-8 flex items-center justify-center hover:bg-gray-50 rounded-full transition-colors text-gray-400 hover:text-red-500">−</button>
                                                <span className="px-4 font-bold text-sm text-gray-700">{product.quantity}</span>
                                                <button onClick={() => dispatch(addToCart(product))} className="w-8 h-8 flex items-center justify-center hover:bg-gray-50 rounded-full transition-colors text-gray-400 hover:text-green-600">+</button>
                                            </div>
                    ...
                    {/* 2. ملخص الحساب (White Modern Card) */}
                    <div className="flex-1 w-full lg:sticky lg:top-10">
                        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.04)]">
                            ...
                            <div className="mt-6 flex items-center justify-center gap-2 text-gray-300">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-widest">Security Guaranteed</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="text-center flex flex-col items-center justify-center">
                    ...
                    <button className="mt-6 text-sm font-black text-black underline tracking-widest hover:text-green-600 transition-colors">CONTINUE SHOPPING</button>
                </div>
            )}
        </div>
    )
